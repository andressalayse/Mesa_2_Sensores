...
// Endereço do multiplexador I2C e canal do sensor de luz da Mesa 2
#define TCA9548A_ADDR               0x70
#define CH_MESA2_BH1750             1
...
//#define BH1750_POS_FACTOR_M2        1.44f   // Correção de posição do sensor na Mesa 2
#define BH1750_POS_FACTOR_M2        4.3f   // Correção de posição do sensor na Mesa 2

// ──────────────────────────────────────────────────────────────────────
// LD2420 — Mesa 2 (UART1 — neste ESP32-C6 dedicado à Mesa 2, o UART1
// está livre, já que não há outro sensor LD2420 disputando o periférico)
// ──────────────────────────────────────────────────────────────────────
#define LD2420_M2_UART_PORT         UART_NUM_1
#define LD2420_M2_PIN_TX            GPIO_NUM_5   // ESP TX → sensor RX
#define LD2420_M2_PIN_RX            GPIO_NUM_4   // ESP RX ← sensor OT1
...
static const char *TAG = "SENSORES_MESA2";
...
// Dados extraídos de um frame do sensor
typedef struct {
    bool     ocupado;
    uint16_t distancia_cm;
    uint8_t  energia;
    int      falhas_consecutivas;   // ciclos sem frame válido — expira o valor
} ld2420_data_t;
...
// Monta e envia um frame de comando ao sensor
...
// Configura distância mínima e máxima de detecção (gates de 0,7 m cada)
...
// ──────────────────────────────────────────────────────────────────────
// Parser modo "Simple" (texto ASCII): o sensor envia linhas como
//   "ON\r\nRange 424\r\n"  ou  "OFF\r\nRange 0\r\n"
// em vez do frame binário "Normal" (AA FF 03 00 ...). Este parser varre
// o buffer texto por linha, procurando o último estado ON/OFF e o
// último valor de Range — o "estado mais recente" no buffer recebido.
// ──────────────────────────────────────────────────────────────────────
static bool ld2420_parse_frame(const uint8_t *buf, int len, ld2420_data_t *out)
{
    ...
    while (i < len) {
        // encontra o fim da linha atual (\r ou \n)
        int start = i;
        while (i < len && buf[i] != '\r' && buf[i] != '\n') i++;
        int line_len = i - start;
        bool linha_completa = (i < len);   // só é completa se terminou em \r ou \n

        // Descarta a primeira linha do buffer: pode estar cortada no início
        // (sobra de um frame anterior).
        if (primeira_linha) {
    ...
    // Só aceita o frame se ON/OFF E Range vierem juntos, completos, na
    // mesma leitura — evita misturar estado de um frame com distância
    // de outro.
    bool achou_algo = achou_estado && achou_dist;

    if (achou_algo) {
        // Filtro de distância: só aceita ocupação se o alvo estiver dentro
        // da faixa de interesse (área da mesa). Fora dessa faixa, força
        // "livre" mesmo que o sensor tenha reportado ON — é presença em
        // outra área (corredor, mesa vizinha, parede refletindo sinal).
        ...
        out->energia      = 0;   // modo Simple não envia energia
    }
    return achou_algo;
}

// Lê o UART do sensor (não bloqueante de mais — timeout curto) e atualiza
// a leitura mais recente em *out. Retorna true se um frame novo foi lido.
// Quantos ciclos sem frame válido até considerar o sensor "sem dados"
// e forçar livre/distância zero (em vez de repetir o último valor antigo)
...
static bool ld2420_ler(uart_port_t uart_port, ld2420_data_t *out)
{
    ...
    // DEBUG: mostra o texto recebido (modo Simple do LD2420 envia ASCII)
...
// Aplica debounce ao estado bruto de ocupação
...
// Inicializa UART + gates de um sensor LD2420
static void ld2420_init(const char *nome, uart_port_t uart_port, int pin_tx, int pin_rx)
{
    ...
    // Se o driver já estiver instalado, remove antes de reinstalar.
    ...
    vTaskDelay(pdMS_TO_TICKS(500));   // aguarda sensor inicializar
    // DEBUG: configuração de gates temporariamente desativada para isolar o problema
...
void app_main(void)
{
    ...
    // Inicializa sensor LD2420 Mesa 2 (UART1 completo)
    ld2420_init("Mesa2", LD2420_M2_UART_PORT, LD2420_M2_PIN_TX, LD2420_M2_PIN_RX);

    // Inicializa sensor de luz BH1750 da Mesa 2
    uint8_t bh_cmd = BH1750_CMD_START;
    tca9548a_select_channel(CH_MESA2_BH1750);
    i2c_master_write_to_device(I2C_PORT, BH1750_SENSOR_ADDR, &bh_cmd, 1, pdMS_TO_TICKS(200));

    ESP_LOGI(TAG, "Inicialização concluída — Mesa 2.");

    // Última leitura válida do sensor Mesa 2 — inicializada como livre
    ld2420_data_t leitura_m2 = { .ocupado = false, .distancia_cm = 0, .energia = 0, .falhas_consecutivas = 0 };

    // Estrutura de debounce — Mesa 2
    ld2420_debounce_t db_m2 = { .estado_atual = LD2420_LIVRE, .cont_ocupado = 0, .cont_livre = 0 };
    ...
    while (1)
    {
        ...
        // Leitura de luminosidade — Mesa 2
        // Fórmula: valor_bruto / 1.2 (conversão padrão BH1750) × fator de
        // calibração do luxímetro de referência × fator de posição da Mesa 2
        tca9548a_select_channel(CH_MESA2_BH1750);
        if (i2c_master_read_from_device(I2C_PORT, BH1750_SENSOR_ADDR, data, 2, pdMS_TO_TICKS(200)) == ESP_OK)
            lux2 = (((data[0] << 8) | data[1]) / 1.2f) * BH1750_CAL_FACTOR * BH1750_POS_FACTOR_M2;

        // Leitura de ocupância — Mesa 2 (LD2420 via UART1)
        ld2420_ler(LD2420_M2_UART_PORT, &leitura_m2);
        int bruto2 = leitura_m2.ocupado ? LD2420_OCUPADO : LD2420_LIVRE;
        int ocupado_mesa2 = ld2420_debounce_atualizar(&db_m2, bruto2);

        // Publicações MQTT
        mqtt_publish_float("mesa2/lux",        lux2);
        mqtt_publish_float("mesa2/distancia",  (float)leitura_m2.distancia_cm);
        mqtt_publish_int  ("mesa2/ocupado",     ocupado_mesa2);

        ESP_LOGI(TAG, "Mesa2 → Lux: %.2f lx | Dist: %u cm | Energia: %u | Ocupada: %d",
                 lux2, leitura_m2.distancia_cm, leitura_m2.energia, ocupado_mesa2);
        ESP_LOGI(TAG, "------------------------------------------------------");
